...
# Check if enough review_phase_outcome events exist for calibration.
# Returns: count on stdout. Exit 0 if >= threshold per phase, 1 otherwise.
# Requires >= threshold events for EACH of the 3 review phases.
_interspect_review_calibration_ready() {
    ...
    local now
    now=$(date +%s)
    local min_phase_count
    min_phase_count=$(sqlite3 "$db" "
        SELECT MIN(cnt) FROM (
            SELECT json_extract(context, '$.phase') as phase, COUNT(*) as cnt
            FROM evidence
            WHERE event = 'review_phase_outcome'
              AND quarantine_until <= $now
            GROUP BY phase
            HAVING phase IN ('brainstorm', 'strategy', 'plan')
        );
    " 2>/dev/null) || min_phase_count=0
    [[ -z "$min_phase_count" ]] && min_phase_count=0

    echo "$min_phase_count"
    [[ "$min_phase_count" -ge "$threshold" ]]
}

# Auto-calibrate review tier-skip thresholds from review_phase_outcome evidence.
# Writes: {project_root}/os/Clavain/config/review-phase-calibration.yaml
# Logic: For each (phase, complexity) pair, compute % of reviews that found P0/P1.
#   If < 5% of reviews at a (phase, complexity) found P0/P1 → mark as skip-eligible.
...
_interspect_calibrate_reviews() {
    ...
    local _project_root
    _project_root=$(dirname "$(dirname "$(dirname "$db")")")
    local output_path="${_project_root}/os/Clavain/config/review-phase-calibration.yaml"

    local now
    now=$(date +%s)
    local calibration_data
    calibration_data=$(sqlite3 -json "$db" "
        SELECT
            json_extract(context, '$.phase') as phase,
            json_extract(context, '$.complexity') as complexity,
            COUNT(*) as total_reviews,
            SUM(CASE WHEN (COALESCE(json_extract(context, '$.p0'), 0) + COALESCE(json_extract(context, '$.p1'), 0)) > 0 THEN 1 ELSE 0 END) as reviews_with_p0p1,
            AVG(COALESCE(json_extract(context, '$.p0'), 0)) as avg_p0,
            AVG(COALESCE(json_extract(context, '$.p1'), 0)) as avg_p1,
            AVG(COALESCE(json_extract(context, '$.agents'), 0)) as avg_agents
        FROM evidence
        WHERE event = 'review_phase_outcome'
          AND quarantine_until <= $now
        GROUP BY phase, complexity
        HAVING total_reviews >= 3
        ORDER BY phase, complexity;
    " 2>/dev/null) || return 1

    [[ -z "$calibration_data" || "$calibration_data" == "[]" ]] && return 0

    # Generate YAML from query results
    local yaml_content
    yaml_content="# Review phase calibration — auto-generated by interspect
# Updated: $(date -u '+%Y-%m-%dT%H:%M:%SZ')
# Source: review_phase_outcome evidence (>= 3 reviews per phase+complexity pair)
#
# Actions: full (run full review), lighten (fewer agents), skip (no review)
# Thresholds: p0p1_rate < 0.05 → skip, 0.05-0.15 → lighten, > 0.15 → full
...
for row in data:
    phase = row['phase']
    complexity = int(row['complexity']) if row['complexity'] else 3
    total = row['total_reviews']
    p0p1 = row['reviews_with_p0p1']
    rate = p0p1 / total if total > 0 else 0
    avg_agents = row['avg_agents'] or 0
    if rate < 0.05:
        action = 'skip'
    elif rate < 0.15:
        action = 'lighten'
        ...
        action = 'full'
    print(f'  {phase}_C{complexity}:')
    print(f'    action: {action}')
    print(f'    p0p1_rate: {rate:.2f}')
    print(f'    total_reviews: {total}')
    print(f'    avg_agents: {avg_agents:.1f}')
" 2>/dev/null) || return 1
...
_interspect_auto_calibrate() {
    ...
    if _interspect_review_calibration_ready 20 >/dev/null 2>&1; then
        _interspect_calibrate_reviews 2>/dev/null || true
    fi
